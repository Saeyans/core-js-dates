...
function getNextFriday(date) {
  ...
  const difference = 5 - day + (day >= 5 ? 7 : 0);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const differenceMilliseconds = new Date(dateEnd) - new Date(dateStart);
  return Math.round(differenceMilliseconds / milliseconds) + 1;
}
